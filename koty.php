...
<body>
    ...
    <div class="container" style="padding:5px">
        ...
        $didbv = 1;
        $sql = 'SELECT produkt_id, nazwa, cena, opis FROM produkt WHERE kategoria_id = :didbv ';
        $stid = oci_parse($conn, $sql);

        oci_bind_by_name($stid, ':didbv', $didbv);
        oci_execute($stid);
        while (($row = oci_fetch_array($stid, OCI_ASSOC)) != false) {
            echo  " <form class='form-horizontal' method='post' action='koszyk.php'><input type='hidden' name='nazwa' value='int'><div class='col-md-12 bg-grey container-fluid prod' style='margin:10px'><b><input type='hidden' name='nazwa' value='text'>" . $row['NAZWA'] . "</b></form><br>\n";
            echo  "<form class='form-horizontal' method='post' action='koszyk.php'><input type='hidden' name='cena' value='int'>Cena: " . $row['CENA'] . "zł</form><br>\n";
            echo "<form class='form-horizontal' method='post' action='koszyk.php'><input type='hidden' name='opis' value='int'>" . $row['OPIS'] . "</form><br>\n <div class='form-group'>
